<template>
  <div class="flex flex-col w-full items-center justify-center px-4">
    ...
    <RouterLink v-if="tag" to="/tags" class="mt-4 text-blue">All Tags</RouterLink>
  </div>
</template>
...
const list = computed(() => (tag.value ? (data.value?.pageData.filter((p) => p.tags.includes(tag.value)) ?? []) : []));
</script>
